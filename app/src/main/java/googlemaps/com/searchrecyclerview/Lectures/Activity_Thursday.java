...
public class Activity_Thursday extends AppCompatActivity {
    private static final String TAG = Activity_Thursday.class.getSimpleName();
    EditText etActivity, etTime, etVenue;
    final Context mContext = this;
    FloatingActionButton fab;
    Thursday_Model thursday_model;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        thursday_model = new Thursday_Model();
        ListView listView = (ListView) findViewById(R.id.assignments_list);
        long count = Thursday_Model.count(Thursday_Model.class);
        if (count > 0) {
            Thursday_Model.listAll(Thursday_Model.class);
            List<Thursday_Model> thursday_models = Thursday_Model.listAll(Thursday_Model.class);
            Thursday_Add_Adapter thursdayAdd_adapter = new Thursday_Add_Adapter(getApplicationContext(), thursday_models);
            listView.setAdapter(thursdayAdd_adapter);
        } else {
    ...
    private void openAlert(View view) {
        ...
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(Activity_Thursday.this);
