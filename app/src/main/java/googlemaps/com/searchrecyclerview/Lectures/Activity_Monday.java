...
public class Activity_Monday extends AppCompatActivity {
    private static final String TAG = Activity_Monday.class.getSimpleName();
    EditText etActivity, etTime, etVenue;
    final Context mContext = this;
    FloatingActionButton fab;
    Monday_Model monday_model;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        monday_model = new Monday_Model();
        ListView listView = (ListView) findViewById(R.id.assignments_list);
        long count = Monday_Model.count(Monday_Model.class);
        if (count > 0) {
            Monday_Model.listAll(Monday_Model.class);
            List<Monday_Model> monday_models = Monday_Model.listAll(Monday_Model.class);
            Monday_Add_Adapter mondayAdd_adapter = new Monday_Add_Adapter(getApplicationContext(), monday_models);
            listView.setAdapter(mondayAdd_adapter);
        } else {
    ...
    private void openAlert(View view) {
        ...
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(Activity_Monday.this);
